...
(defn- physics [world dt]
  (let [vy' (+ (:bird-vy world) (* gravity dt))
        vy'' (if (flap-pressed?) flap-vy vy')
        y'   (+ (:bird-y world) (* vy'' dt))]
    (assoc world :bird-y y' :bird-vy vy'')))
...
(defn- move-pipes [{:keys [pipes score] :as world} dt]
  (let [moved     (mapv (fn [p] (update p :x - (* pipe-speed dt))) pipes)
        ...
        kept      (if (< rightmost (- width pipe-spacing))
                    (conj kept (new-pipe (+ rightmost pipe-spacing)))
                    kept)]
    (assoc world :pipes kept :score score')))

(defn- collides? [bird-y pipes]
  (or (< bird-y bird-radius) (> bird-y (- height bird-radius))
      (some (fn [{:keys [x gap-y]}]
              (and (< (- bird-x bird-radius) (+ x pipe-w)) (> (+ bird-x bird-radius) x)
                   (or (< (- bird-y bird-radius) (- gap-y (/ pipe-gap 2)))
                       (> (+ bird-y bird-radius) (+ gap-y (/ pipe-gap 2))))))
            pipes)))
