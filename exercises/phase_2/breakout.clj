...
(defn- move-paddle [world dt]
  (let [delta (* paddle-speed dt)]
    (cond
      (or (keyboard/is-key-down? (:a enums/keyboard-key))
          (keyboard/is-key-down? (:left enums/keyboard-key)))
      (update world :paddle-x #(clamp (- % delta) 0 (- width paddle-width)))
      (or (keyboard/is-key-down? (:d enums/keyboard-key))
...
(defn- brick-hit-index [bricks x y]
  (->> (range (* rows cols))
       (filter #(nth bricks %))
       (map (fn [i] [i (brick-rect i)]))
       (filter (fn [[_ r]]
                 (and (>= (+ x ball-radius) (:x r)) (<= (- x ball-radius) (+ (:x r) (:w r)))
                      (>= (+ y ball-radius) (:y r)) (<= (- y ball-radius) (+ (:y r) (:h r))))))
       ffirst))
...
(defn- move-ball [{:keys [paddle-x ball bricks] :as world} _dt]
  (let [{:keys [x y dx dy]} ball
    ...
    (cond
      ...
      (let [hit-ratio (clamp (/ (- x2 paddle-x) paddle-width) 0.0 1.0)
            new-dx    (* ball-speed (- (* 2 hit-ratio) 1.0))]
